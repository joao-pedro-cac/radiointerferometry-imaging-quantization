...
class ImagePipeline():
    ...
    def set_truth_model(self, truth_model_path):
        self.truth_image = np.squeeze(fits.getdata(truth_model_path))    # truth (original) sky image
        self.__npix_x, self.__npix_y, self.__pixel_size_x, self.__pixel_size_y = extract_pixel_info(truth_model_path)
    ...
    def calculate_image_metrics(self, clean_image):
        image_metrics = {
            ...
            "snr"           : float(image_data_analysis.compute_snr(clean_image, self.truth_image)),
            "psnr"          : float(image_data_analysis.compute_psnr(clean_image, self.truth_image)),
            "ssim"          : float(image_data_analysis.compute_ssim(clean_image, self.truth_image))
        }
